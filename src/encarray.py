...
class EncArray:
    ...
    def __matmul__(self, other):
        ...
        result = [[EncArray([
            (EncArray(ele_a, enc_utils=self.enc_utils, is_encrypted=True) * EncArray(ele_b, enc_utils=self.enc_utils, is_encrypted=True)).enc_arr
            for ele_a, ele_b in zip(row_a, col_b)],
            enc_utils=self.enc_utils, is_encrypted=True).sum().enc_arr
